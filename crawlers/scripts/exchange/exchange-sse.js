// 上交所公告采集脚本
// 使用 Hermes Browser Tool 采集上交所最新公告

const sseAnnouncements = [];
...
for (let link of allLinks) {
  ...
  if (text.length > 10 && text.length < 200 &&
      ...
      href && href.length > 20 &&
      (href.includes('sse.com.cn') || href.includes('/disclosure/'))) {
    ...
    if (text.includes('公告') || text.includes('通知') || text.includes('披露')) {
      category = '上市公司公告';
      importance = 'high';
    } else if (text.includes('停牌') || text.includes('复牌')) {
      category = '停复牌信息';
      importance = 'high';
...
for (let item of sseAnnouncements) {
...
if (typeof document === 'undefined' && typeof module !== 'undefined' && module.exports !== undefined) {
  ...
  if (outputDir) {
    ...
    const outputFile = path.join(outputDir, `sse-${timestamp}.json`);
