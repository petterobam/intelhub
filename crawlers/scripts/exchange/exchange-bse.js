// 北交所公告采集脚本
// 使用 Hermes Browser Tool 采集北交所最新公告

const bseAnnouncements = [];
...
for (let link of allLinks) {
  ...
  if (text.length > 10 && text.length < 200 &&
      ...
      href && href.length > 20 &&
      (href.includes('bse.cn') || href.includes('/disclosure/'))) {
    ...
    if (text.includes('公告') || text.includes('通知') || text.includes('披露')) {
      ...
      category = '监管处罚';
      importance = 'high';
    } else if (text.includes('申购') || text.includes('新股') || text.includes('上市')) {
      category = '新股发行';
      importance = 'medium';
    }

    bseAnnouncements.push({
      title: text,
      url: href,
      category: category,
...
for (let item of bseAnnouncements) {
