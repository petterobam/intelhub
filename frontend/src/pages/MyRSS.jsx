...
import { Rss, Youtube, Tv, Plus, Trash2, RefreshCw, Power, PowerOff, Loader2, CheckCircle, AlertCircle, X } from 'lucide-react'
...
export default function MyRSS() {
  ...
  return (
    <TierGuard minTier="v3">
    <div className="space-y-6 w-full">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-white">我的 RSS</h2>
          <p className="text-xs text-slate-500 mt-1">管理个人内容订阅源</p>
        </div>
...
function AddSourceModal({ onClose, onSaved }) {
  ...
  const handleSave = async () => {
    ...
    setError('')
    try {
      await api.post('/api/v1/user-sources', {
      ...
      <div className="bg-slate-800 rounded-xl p-6 w-full max-w-md shadow-2xl border border-slate-700" onClick={e => e.stopPropagation()}>
        ...
        <div className="space-y-4">
          ...
          <div className="flex gap-3 pt-2">
            <button onClick={handleSave} disabled={validating || !sourceId.trim()}
